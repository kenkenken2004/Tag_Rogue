...
URogueBeta_MapGenerator::URogueBeta_MapGenerator(int32 MapHeight, int32 MapWidth) :UMapGeneratorBase(MapHeight,MapWidth) {
	...
	//
	for (int32 i = 0; i < AreaList.Num(); i++) {
		FCell* Cell1 = GetCell(AreaList[i]->LeftTopCell->Px + 1, AreaList[i]->LeftTopCell->Py + 1);//作る部屋の左上のCell
		FCell* Cell2 = GetCell(AreaList[i]->RightBottomCell->Px - 1, AreaList[i]->RightBottomCell->Py - 1);//作る部屋の右下のCell
